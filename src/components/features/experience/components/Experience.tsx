    ...
    <section id="experience" className="relative py-24 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <SectionHeading
          ...
          title={
            <>
              Experience that <span className="text-gradient">delivers real impact</span>
            </>
          }
        ...
        <div className="relative mx-auto max-w-3xl">
          <div className="absolute left-5 top-2 bottom-2 w-px bg-gradient-to-b from-cyan-400/60 via-violet-500/40 to-transparent" />

          <div className="space-y-10">
            {profile.experience.map((job, i) => (
              <Reveal key={job.company} delay={i * 0.1} variant="left" className="relative pl-16">
                <span className="absolute left-0 top-1 flex h-11 w-11 items-center justify-center rounded-full border border-cyan-400/30 bg-ink-800 shadow-[0_0_18px_rgba(34,211,238,0.25)]">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {profile.workProjects.map((project, i) => (
              <Reveal key={project.name} delay={(i % 3) * 0.08} variant="up">
                <div className="glass group flex h-full flex-col rounded-3xl p-6 transition-all hover:-translate-y-1 hover:border-violet-400/30">
                  ...
                  <p className="mb-4 text-sm font-semibold text-white">{project.tagline}</p>
                  <ul className="mb-6 flex-1 space-y-2">
                    {project.overview.slice(0, 2).map((o) => (
                      <li key={o.slice(0, 40)} className="text-sm leading-relaxed text-slate-400">
                        {o}
                      </li>
                    ))}
                  </ul>
