...
import { Award, HeartHandshake, Languages } from "lucide-react";
          ...
          <Reveal variant="left">
            <div className="space-y-5">
              {profile.education.map((edu) => (
                <div
                  key={edu.degree}
                  className="glass rounded-3xl p-6 transition-colors hover:border-cyan-400/25 sm:p-7"
                >
                  <div className="flex items-start gap-4">
                    <span className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-transparent p-1.5">
              ...
              <div className="glass rounded-3xl p-6 sm:p-7">
                ...
                <div className="flex flex-wrap gap-3">
                  {profile.languages.map((lang) => (
                    <span
                      key={lang.name}
                      className="rounded-full border border-white/10 bg-white/4 px-4 py-2 text-sm text-slate-300"
                    >
              ...
              <div className="glass rounded-3xl p-6 sm:p-7">
                ...
                <div className="space-y-3">
                  {profile.training.map((t) => (
                    <div
                      key={t.name}
                      className="flex items-center justify-between gap-4 rounded-2xl border border-white/5 bg-white/2 px-4 py-3"
                    >
                      <div>
